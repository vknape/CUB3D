...
/*   Updated: 2025/02/06 12:03:46 by snijhuis      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void ray_end_ypos(t_all *all)
{
	if (all->ray->yneg)
	{
		...
		if (all->ray->xneg)
			...
			all->ray->calc_x += all->ray->calc_y / tan(all->ray->calc_or);
		all->ray->calc_y = floor(all->game->py);
		all->ray->calc_y -= 1;
		...
		if (all->parse->map[(int)floor(all->ray->calc_y)][(int)floor(all->ray->calc_x)] == wall)
			if (all->ray->valid_ray == false || all->ray->calc_ray < all->ray->ray_length)
	...
	else
	{
		...
		if (all->ray->xneg)
			...
			all->ray->calc_x += all->ray->calc_y / tan(all->ray->calc_or);
		all->ray->calc_y = ceil(all->ray->calc_y) + floor(all->game->py);
		if (all->ray->calc_x >= all->parse->map_width || all->ray->calc_y >= all->parse->map_height || all->ray->calc_x < 0 || all->ray->calc_y < 0)
			return(assign_ray(all, 2));
		if (all->parse->map[(int)floor(all->ray->calc_y)][(int)floor(all->ray->calc_x)] == wall)
			if (all->ray->valid_ray == false || all->ray->calc_ray < all->ray->ray_length)
